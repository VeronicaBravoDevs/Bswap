...
export class ExchangeService {
  ...
  async create(createExchangeDto: CreateExchangeDto) {
    try {
      ...
      const ownerEmail = bookExist.userId;
      ...
      const dataUserBook = await this.prismaService.user.findUnique({
        where: { id: ownerEmail },
      });
  ...
  async update(id: string, updateExchangeDto: UpdateExchangeDto) {
    try {
      const { requesterId, bookId, request_date, status } =
        updateExchangeDto as {
          requesterId: string;
          bookId: string;
          request_date: Date;
